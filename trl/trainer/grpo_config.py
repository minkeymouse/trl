...
class GRPOConfig(_BaseConfig):
    ...
    Parameters:
        ...
        vllm_trust_remote_code (`bool`, *optional*):
            Passed to vLLM's `LLM(..., trust_remote_code=...)` in colocate mode. If `None` (default), TRL infers from the
            model config (`trust_remote_code` or `auto_map`), which is required for models such as Nemotron-H.
    ...
    vllm_trust_remote_code: bool | None = field(
        ...
        metadata={
            ...
            "inferred from the model config (e.g. remote-code / Nemotron-H)."
        },
    )
